...
"""
def cast_values_mass(lista:list[dict],key:str="mass")->int:
    for i in range(len(lista)):
        for elem in lista[i][key]:
            list_rtn = []
            if key == re.findall(["0-9"],lista[i][key]):
                rtn = list(map(int,key))
                list_rtn.append(rtn)
            return list_rtn
"""     
...
#? INFO
def sort_list(lista:list[dict],key:str) -> list[dict]:
    to_sort_list = lista.copy()
    to_sort_list.sort()
